...
int checkGlError( const std::string& aInfo, const char* aFile, int aLine, bool aThrow )
{
    ...
    // Note: GL_STACK_UNDERFLOW/OVERFLOW are legacy OpenGL 1.x matrix stack errors.
    // WebGL uses shader-based rendering with no matrix stack - these never occur.
...
// Note: GL_DEBUG_OUTPUT and glDebugMessageCallback are desktop OpenGL extensions
// not available in WebGL. Debug output is not supported.


void enableGlDebug( bool aEnable )
{
    // GL debug output not available in WebGL - no-op
    (void)aEnable;
}
